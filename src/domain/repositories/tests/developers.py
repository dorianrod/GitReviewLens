...
async def test_it_is_race_condition_proof(developer_repository, fixture_developer_dict):
    async def upsert_developer():
        await developer_repository.upsert(Developer.from_dict(fixture_developer_dict))
...
async def test_it_gets_developer(developer_repository, create_developper):
...
async def test_it_creates_developer(create_developper, developer_repository):
    created_developper = await create_developper()
    assert await developer_repository.find_all() == [created_developper]


async def test_it_creates_developer_with_upsert(
    developer_repository, fixture_developer_dict
):
    developer = Developer.from_dict(fixture_developer_dict)
...
async def test_it_raises_exception_if_create_an_existing_developer(
    developer_repository, create_developper, fixture_developer_dict
):
...
async def test_it_updates_developer(
    developer_repository, fixture_developer_dict, create_developper
):
    await create_developper()
