...
class MixinFeatures:
    async def test_upsert(self, feature_repository, fixture_feature_dict):
    ...
    async def test_cannot_create_into_wrong_git_repo(
        self, feature_repository, fixture_feature_dict
    ):
    ...
    async def test_create_same_id_in_two_repos(
        self, feature_repository, feature_repository_2, fixture_feature_dict
    ):
        feature = Feature.from_dict(fixture_feature_dict)
        await feature_repository.create(feature)
        ...
        feature_2 = Feature.from_dict(
            {
                **fixture_feature_dict,
                "git_repository": feature_repository_2.git_repository,
            }
        )
    ...
    async def test_update(self, feature_repository, fixture_feature_dict):
        feature = Feature.from_dict(fixture_feature_dict)
        await feature_repository.create(feature)
    ...
    async def test_upsert_all(self, feature_repository, fixture_feature_dict):
        feature = Feature.from_dict(fixture_feature_dict)
        await feature_repository.upsert_all([feature])

        assert await feature_repository.find_all() == [feature]

    async def test_find_all(self, feature_repository, fixture_feature_dict):
        feature = Feature.from_dict(fixture_feature_dict)
        await feature_repository.create(feature)
